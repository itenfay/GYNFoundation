...
public let FYDURLErrorTimeout = NSURLErrorTimedOut
...
public let FYDURLErrorDisconnection = NSURLErrorNotConnectedToInternet

open class FYDHttpClient: NSObject, URLSessionTaskDelegate, URLSessionDataDelegate {
    ...
    private func configureURLRequest() -> URLRequest {
        ...
        print("[configureURLRequest] >>> aUrl: \(aUrl)")
    ...
    private func acceptableContentValue() -> String {
        ...
        print(" >>> acceptableContentValue: " + result)
    ...
    open func urlSession(_ session: URLSession, task: URLSessionTask, willPerformHTTPRedirection response: HTTPURLResponse, newRequest request: URLRequest, completionHandler: @escaping (URLRequest?) -> Void) {
        ...
        print("[urlSession] >>> willPerformHTTPRedirection response: ")
    }
    
    open func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        print("[urlSession] >>> didReceive response: ")
        self.recvData?.length = 0
        let httpUrlResponse = response as! HTTPURLResponse
        
        if httpUrlResponse.statusCode == FYDURLNoError {
            completionHandler(URLSession.ResponseDisposition.allow)
        } else {
            completionHandler(URLSession.ResponseDisposition.cancel)
            
            let error = NSError(domain: self.url?.host ?? "fyd.urlresponse.status",
    ...
    open func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        print("[urlSession] >>> didReceive data: ")
    ...
    open func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil {
            print("[urlSession] >>> didCompleteWithError error: \(error!) ")
